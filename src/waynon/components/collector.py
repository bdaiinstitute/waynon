...
from waynon.utils.utils import COLORS, Cancellable
...
class CollectorData(Component):
    ...
    def draw_property(self, nursery, entity_id):
        self.draw_collector(nursery, entity_id)

    def model_post_init(self, __context):
        self._collect_cancellable = None

    def draw_collector(self, nursery: trio.Nursery, collector_id: int):
            ...
            if self._collect_cancellable is None:
                self._collect_cancellable = Cancellable(
                    nursery,
                    "Move & Collect",
            ...
            self._collect_cancellable.draw((imgui.get_content_region_avail().x, 40))
